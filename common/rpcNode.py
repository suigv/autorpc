...
# 返回值:整型1表示成功0表示失败
    def get_node_nound(self):
        ...
        if self._handle!=0:
            ...
            r = ctypes.c_int(0)
            b = ctypes.c_int(0)
            exec_ret = self._rpc.getNodeNound(self._handle, ctypes.byref(l), ctypes.byref(t), ctypes.byref(r), ctypes.byref(b))
            if exec_ret == 1:
                ret['left'] = l.value
                ret['top'] = t.value
                ret['right'] = r.value
...
# 返回值:整型1表示成功0表示失败
    def get_node_nound_center(self):
        ret = {'x':-1, 'y':-1}
        if self._handle!=0:
            ...
            x = ctypes.c_int(0)
            y = ctypes.c_int(0)
            exec_ret = self._rpc.getNodeNoundCenter(self._handle, ctypes.byref(x), ctypes.byref(y))
            if exec_ret == 1:
                ret['x'] = x.value
                ret['y'] = y.value
        return ret
